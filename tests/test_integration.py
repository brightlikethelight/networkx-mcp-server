...
class TestEndToEndWorkflows:
    ...
    async def test_complete_analysis_workflow(self, temp_files):
        ...
        from networkx_mcp.server import (
            import_graph, centrality_measures, community_detection, 
            visualize_graph, export_graph, graph_info
        )
    ...
    async def test_social_network_analysis_workflow(self):
        ...
        from networkx_mcp.server import (
            create_graph, add_nodes, add_edges, centrality_measures,
            community_detection, graph_info
        )
    ...
    async def test_transport_network_workflow(self):
        ...
        from networkx_mcp.server import (
            ...
            minimum_spanning_tree, graph_info
        )
    ...
    async def test_research_citation_workflow(self):
        ...
        from networkx_mcp.server import (
            create_graph, add_nodes, add_edges, centrality_measures,
            connected_components, graph_info
        )
...
class TestComplexDataIntegration:
    ...
    def test_heterogeneous_data_integration(self, temp_files):
        ...
        try:
            ...
            with open(node_attrs_file.name, 'r') as f:
                attrs = json.load(f)
    ...
    def test_temporal_data_integration(self):
        ...
        for source, target, attrs in temporal_data:
            if not temporal_graph.has_edge(source, target):
                ...
                if attrs["timestamp"] > temporal_graph.edges[source, target]["timestamp"]:
        ...
        for u, v, data in temporal_graph.edges(data=True):
