...
class TestResourceMonitor:
    ...
    def test_memory_limit_check(self):
        ...
        # Mock memory usage
        with patch.object(
            monitor, "_get_memory_usage", return_value=100 * 1024 * 1024
        ):  # 100MB
            assert monitor.check_memory_limit("user1") == True

        # Mock high memory usage
        with patch.object(
            monitor, "_get_memory_usage", return_value=600 * 1024 * 1024
        ):  # 600MB
...
class TestRateLimiter:
    ...
    def test_rate_limiter_creation(self, mock_config):
        ...
        with patch(
            "networkx_mcp.enterprise.rate_limiting.get_config", return_value=mock_config
        ):
            limiter = RateLimiter()
    ...
    def test_rate_limit_disabled(self):
        ...
        with patch(
            "networkx_mcp.enterprise.rate_limiting.get_config", return_value=config
        ):
            limiter = RateLimiter()
    ...
    def test_resource_limits(self, mock_config):
        ...
        with patch(
            ...
            # Mock high memory usage
            with patch.object(
                limiter.resource_monitor, "check_memory_limit", return_value=False
            ):
                result = limiter.check_rate_limit("user1")
                assert result.allowed == False
    ...
    def test_operation_tracking_context(self, mock_config):
        ...
        with patch(
            ...
            with limiter.operation_tracking("user1", "pagerank") as operation_id:
                ...
                # Operation should be tracked
                assert (
                    operation_id.split("_")[0] + "_" + operation_id.split("_")[1]
                    == "user1_pagerank"
                )
...
class TestRateLimitingIntegration:
    ...
    def test_rate_limiting_with_metrics(self, mock_config):
        ...
        with patch(
            ...
            with patch(
                "networkx_mcp.enterprise.rate_limiting.get_metrics_collector",
    ...
    def test_rate_limiting_with_audit_logging(self, mock_config):
        ...
        with patch(
            ...
            with patch(
                "networkx_mcp.enterprise.rate_limiting.get_audit_logger",
