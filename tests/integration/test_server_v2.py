...
def test_mock_mcp():
    ...
    async def test_resource():
        ...
        return MockMCP.types.TextResourceContent(
            uri="test://resource", mimeType="text/plain", text="Test resource content"
        )
    ...
    async def test_prompt(name: str = "World"):
        """Test prompt."""
        return [
            MockMCP.types.TextContent(type="text", text=f"Hello {name} from prompt!")
        ]
...
def test_resource_definitions():
    ...
    try:
        ...
        with open("src/networkx_mcp/server/resources/__init__.py") as f:
            content = f.read()
...
def test_prompt_definitions():
    ...
    try:
        ...
        with open("src/networkx_mcp/server/prompts/__init__.py") as f:
            content = f.read()
