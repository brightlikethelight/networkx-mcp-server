        ...
        try:
            ...
            for attr in common_attrs:
                if hasattr(server, attr):
                    assert (
                        getattr(server, attr) is not None
                        or getattr(server, attr) is not None
                    )
    ...
    def test_validation_module_detailed(self):
        ...
        try:
            ...
            if hasattr(validator, "validate_graph_operation"):
                result = validator.validate_graph_operation(
                    "create_graph", {"name": "test"}
                )
                assert isinstance(result, (bool, dict))
...
class TestVisualizationModulesDetailed:
    ...
    def test_matplotlib_visualizer_detailed(
        self, mock_show, mock_close, mock_savefig, mock_figure
    ):
    ...
    def test_validators_module_detailed(self):
        ...
        try:
            ...
            for func_name in validation_functions:
                try:
                    func = getattr(
                        __import__("networkx_mcp.utils.validators"), func_name
                    )
                    if callable(func):
                        result = func(graph)
                        assert result is not None or result is None
