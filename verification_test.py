...
async def test_mcp_protocol(server, results):
    ...
    results.add_result("MCP Initialize", success, 
                      None if success else "Invalid initialize response")
    ...
    results.add_result("Tool Count (≥20)", len(tools) >= 20,
    ...
    for tool in tools:
        ...
        if not has_schema:
            results.add_result(f"Tool Schema: {tool.get('name', 'unknown')}", False,
...
async def test_graph_operations(server, results):
    ...
    result = await call_tool(server, "create_graph", {"name": "test_directed", "directed": True})
    results.add_result("Create Directed Graph", 
                      result.get("type") == "directed")
    
    # Create undirected graph
    result = await call_tool(server, "create_graph", {"name": "test_undirected", "directed": False})
    results.add_result("Create Undirected Graph", 
                      result.get("type") == "undirected")
    ...
    results.add_result("Get Graph Info", 
                      result.get("nodes") == 5 and result.get("edges") == 6)
    ...
    try:
        ...
        results.add_result("Error Handling: Non-existent Graph", False, 
                          "Should have raised error")
    except Exception as e:
        results.add_result("Error Handling: Non-existent Graph", 
...
async def test_algorithms(server, results):
    ...
    centralities = result.get("centrality", {})
    results.add_result("Betweenness Centrality", 
                      "D" in centralities,
    ...
    if "largest_component" in result:
        results.add_result("Connected Components", 
                          result.get("num_components") == 1 and 
                          len(result.get("largest_component", [])) == 6)
    else:
        components = result.get("components", [])
        results.add_result("Connected Components", 
                          len(components) == 1 and len(components[0]) == 6)
    
    # Test community detection
    try:
        result = await call_tool(server, "community_detection", {"graph": "algo_test"})
        communities = result.get("communities", [])
        results.add_result("Community Detection", 
                          len(communities) > 0 and 
                          sum(len(c) for c in communities) == 6)
    except Exception as e:
...
Node1,Node3"""
    ...
    result = await call_tool(server, "import_csv", {
        "graph": "csv_test",
        "csv_data": csv_data,
        "directed": False
    })
    results.add_result("CSV Import", 
                      result.get("nodes") == 4 and result.get("edges") == 5)
    ...
    results.add_result("JSON Export", 
                      "nodes" in json_data and "links" in json_data and
                      len(json_data["nodes"]) == 4)
...
3,1"""
    ...
    result = await call_tool(server, "import_csv", {
        "graph": "csv_numeric",
        "csv_data": csv_numeric,
        "directed": True
    })
    results.add_result("CSV Import (Numeric Nodes)", 
...
async def test_academic_features(server, results):
    ...
    try:
        result = await call_tool(server, "resolve_doi", {
            "doi": "10.1038/nature12373"  # Higgs boson paper
        })
        results.add_result("DOI Resolution", 
                          "title" in result and "authors" in result)
    except Exception as e:
        results.add_result("DOI Resolution", False, 
                          f"Network error or API issue: {str(e)}")
    ...
    try:
        ...
        results.add_result("Build Citation Network", 
                          "papers_added" in result or "nodes" in result)
    except Exception as e:
        results.add_result("Build Citation Network", False,
                          f"API or network issue: {str(e)}")
    ...
    await call_tool(server, "add_edges", {
        "graph": "mock_citations",
        "edges": [
            ["Paper1", "Paper2"], ["Paper1", "Paper3"],
            ["Paper2", "Paper4"], ["Paper3", "Paper4"],
            ["Paper4", "Paper5"]
        ]
    })
    ...
    try:
        result = await call_tool(server, "analyze_author_impact", {
            "graph": "mock_citations",
            "author_name": "Smith"
        })
        # Should return something even if no data
        results.add_result("Analyze Author Impact", 
    ...
    try:
        result = await call_tool(server, "find_collaboration_patterns", {
            "graph": "mock_citations"
        })
        results.add_result("Find Collaboration Patterns", 
                          "patterns" in result or "clusters" in result or 
                          "No collaboration" in str(result))
    except Exception as e:
        results.add_result("Find Collaboration Patterns", False, str(e))
    
    # Test research trends
    try:
        result = await call_tool(server, "detect_research_trends", {
            "graph": "mock_citations",
            "time_window": 3
        })
        results.add_result("Detect Research Trends", 
                          "trends" in result or "error" in str(result))
    except Exception as e:
        results.add_result("Detect Research Trends", True,
                          "Expected - needs temporal data")
    
    # Test BibTeX export
    try:
        result = await call_tool(server, "export_bibtex", {
            "graph": "mock_citations"
        })
        results.add_result("Export BibTeX", 
                          "bibtex" in result or "@" in str(result))
    except Exception as e:
        results.add_result("Export BibTeX", False, str(e))
    
    # Test paper recommendations
    try:
        result = await call_tool(server, "recommend_papers", {
            "graph": "mock_citations",
            "seed_doi": "Paper1",
            "max_recommendations": 5
        })
        results.add_result("Recommend Papers", 
                          "recommendations" in result or "error" in str(result))
    except Exception as e:
...
async def test_performance_scalability(server, results):
    ...
    results.add_result("Small Graph Creation (100 nodes)", True,
    ...
    results.add_result("PageRank on Small Graph", True,
                      performance=f"{small_pagerank_time:.3f}s")
    ...
    results.add_result("Medium Graph Creation (1000 nodes)", True,
    ...
    results.add_result("Shortest Path on Medium Graph", 
                      "path" in result,
...
async def test_edge_cases(server, results):
    ...
    results.add_result("Empty Graph Info", 
                      result.get("nodes") == 0 and result.get("edges") == 0)
    
    # Shortest path on disconnected nodes
    await call_tool(server, "add_nodes", {"graph": "empty_graph", "nodes": ["X", "Y"]})
    try:
        await call_tool(server, "shortest_path", {
            "graph": "empty_graph",
            "source": "X",
            "target": "Y"
        })
        results.add_result("Shortest Path (Disconnected)", False,
    ...
    try:
        ...
        results.add_result("Invalid Tool Name", False,
                          "Should raise error")
    except Exception as e:
        results.add_result("Invalid Tool Name", 
                          "unknown tool" in str(e).lower())
    
    # Missing required parameters
    try:
        await call_tool(server, "add_nodes", {"graph": "test"})  # Missing nodes
        results.add_result("Missing Required Parameter", False,
                          "Should raise error")
    except Exception as e:
...
Eve,Frank"""
    ...
    degree_result = await call_tool(server, "degree_centrality", 
                                   {"graph": "social_network"})
    
    # 4. Detect communities
    try:
        comm_result = await call_tool(server, "community_detection", 
                                     {"graph": "social_network"})
        has_communities = "communities" in comm_result
    except:
        has_communities = False
    
    # 5. Visualize
    try:
        viz_result = await call_tool(server, "visualize_graph", {
            "graph": "social_network",
            "layout": "spring"
        })
    except Exception:
    ...
    # 6. Export as JSON
    export_result = await call_tool(server, "export_json", 
                                   {"graph": "social_network"})
    
    workflow_success = (
        result.get("nodes", 0) > 0 and
        "centrality" in degree_result and
        ("visualization" in viz_result or "image" in viz_result) and
        ("json" in export_result or "graph_data" in export_result)
    )
    ...
    path_result = await call_tool(server, "shortest_path", {
        "graph": "transport_network",
        "source": "Airport",
        "target": "University"
    })
    ...
    components = await call_tool(server, "connected_components", 
                                {"graph": "transport_network"})
    
    transport_success = (
        path_result.get("path", []) == ["Airport", "Downtown", "University"] and
        (components.get("num_components", 0) > 0 or len(components.get("components", [])) > 0)
    )
...
async def test_all_tools_exist(server, results):
    ...
    for tool_name in expected_tools:
        results.add_result(f"Tool exists: {tool_name}", 
                          tool_name in available_tools)
    
    # Verify we have at least 20 tools
    results.add_result("Total tool count ≥ 20", 
