...
def main():
    ...
    for r in results:
        ...
        print(f"{r['type']:<15} {r['memory']:<12.1f} {r['startup_time']:<12.2f} {status:<10}")
    ...
    if len(results) == 2:
        ...
        print("SAVINGS")
        print(f"{'='*60}")
        print(f"Memory saved: {mem_saved:.1f}MB ({mem_saved/results[1]['memory']*100:.0f}% reduction)")
        print(f"Startup time saved: {time_saved:.2f}s ({time_saved/results[1]['startup_time']*100:.0f}% faster)")
