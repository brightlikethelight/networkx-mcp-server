        ...
        async def graph_stats(graph_id: str) -> ResourceContent:
            ...
            stats = {
                ...
                "connectivity": {
                    ...
                    "number_connected_components": nx.number_connected_components(graph) if not graph.is_directed() else nx.number_weakly_connected_components(graph)
                },
                "degree": {
                    "average_degree": sum(d for n, d in graph.degree()) / graph.number_of_nodes() if graph.number_of_nodes() > 0 else 0,
