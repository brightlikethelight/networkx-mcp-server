...
class WebhookMonitor:
    ...
    async def analyze_failures(self, window_hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            'top_failing_workflows': dict(sorted(
                workflow_failures.items(),
                key=lambda x: x[1],
                reverse=True
            )[:5]),
            'top_failing_branches': dict(sorted(
                branch_failures.items(),
                key=lambda x: x[1],
                reverse=True
            )[:5]),
...
class AlertAggregator:
    ...
    def get_aggregated_alert(self, key: str) -> Optional[Alert]:
        ...
        aggregated = Alert(
            id=first_alert.id,
            severity=max(a.severity for a in alerts, key=lambda s: list(Severity).index(s)),
            title=f"{first_alert.title} ({len(alerts)} occurrences)",
            message=f"Multiple failures detected:\n" + "\n".join(set(a.message for a in alerts)),
