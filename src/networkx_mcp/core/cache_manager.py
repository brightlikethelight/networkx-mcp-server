...
class AdaptiveEvictionStrategy:
    ...
    def select_eviction_candidates(
        ...
        # Calculate scores
        scored_entries = [
            (entry, self.calculate_eviction_score(entry))
...
class CacheManager:
    ...
    def __init__(
        ...
        logger.info(f"Initialized cache manager with {max_size_mb}MB limit, "
    ...
    async def get(
        ...
        **kwargs
        ...
        if compute_func:
            ...
            value = await compute_func(*args, **kwargs) if asyncio.iscoroutinefunction(compute_func) else compute_func(*args, **kwargs)
