...
class GraphMemoryManager:
    ...
    def can_allocate(self, size_bytes: int) -> bool:
        ...
        return (self.current_usage + size_bytes <= self.max_memory and 
                size_bytes <= system_limit)
    
    def allocate(self, graph_id: str, size_bytes: int):
        """Allocate memory for a graph"""
        self.graph_sizes[graph_id] = size_bytes
        self.current_usage += size_bytes
        logger.debug(f"Allocated {size_bytes} bytes for graph {graph_id}. "
...
class ThreadSafeGraphStore:
    ...
    @asynccontextmanager
    async def acquire_lock(self, graph_id: str, lock_type: LockType, owner: str = "system"):
        """
    ...
    async def create_graph(
        ...
        async with self.acquire_lock(graph_id, LockType.EXCLUSIVE, owner or "system"):
            ...
            logger.info(f"Created graph {graph_id} with {graph.number_of_nodes()} nodes, "
    ...
    async def update_graph(
        ...
        async with self.acquire_lock(graph_id, LockType.WRITE, owner or "system"):
            ...
            try:
                ...
                self.metadata[graph_id].node_count = self.graphs[graph_id].number_of_nodes()
                self.metadata[graph_id].edge_count = self.graphs[graph_id].number_of_edges()
                ...
                if new_size > old_size:
                    ...
                    if not self.memory_manager.can_allocate(new_size - old_size):
                        ...
                        raise MemoryError(f"Insufficient memory for graph update")
    ...
    def _invalidate_cache(self, graph_id: str):
        ...
        keys_to_remove = [
            key for key in self.computation_cache
            if key.startswith(f"{graph_id}:")
        ]
