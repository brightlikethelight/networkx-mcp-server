...
class EnterpriseNetworkXServer:
    ...
    async def handle_request(self, request: dict) -> dict:
        ...
        try:
            ...
            self.metrics.record_auth_attempt(
                "api_key" if "_api_key" in str(request) else "oauth", user is not None
            )
            ...
            self.metrics.record_request(
                request.get("method", "unknown"), duration, "error"
            )
            self.metrics.record_error(request.get("method", "unknown"), error_type)
    ...
    async def _handle_initialize(self, params: dict) -> dict:
        ...
        return {"jsonrpc": "2.0", "id": params.get("id"), "result": server_info}
    ...
    async def _handle_tools_list(self, params: dict) -> dict:
        ...
        with self.audit.operation_context("list_tools"):
            ...
            allowed_operations = self.auth_manager.rbac_manager.get_allowed_operations(
                self._current_user
            )
            ...
            filtered_tools = [
                tool for tool in all_tools if tool["name"] in allowed_operations
            ]
    ...
    async def _handle_tool_call(self, params: dict) -> dict:
        ...
        rate_result = self.rate_limiter.check_rate_limit(
            self._current_user.id, tool_name
        )
        ...
        try:
            with self.rate_limiter.operation_tracking(self._current_user.id, tool_name):
                with self.audit.operation_context(
                    tool_name, args.get("graph", "unknown")
                ):
            ...
            if tool_name == "create_graph":
                ...
                if graph_name in graphs:
                    ...
                    self.metrics.record_graph_metrics(
                        len(graphs), graph.number_of_nodes(), graph.number_of_edges()
                    )
            ...
            self.audit.log_operation(
                tool_name, args.get("graph", "unknown"), False, duration
            )
            raise
            ...
            if tool_name == "add_nodes":
                ...
                if len(nodes) > self.config.rate_limit.max_graph_size:
                    raise ValidationError(
                        f"Too many nodes (max: {self.config.rate_limit.max_graph_size})"
                    )
                ...
                if len(edges) > self.config.rate_limit.max_graph_size:
                    raise ValidationError(
                        f"Too many edges (max: {self.config.rate_limit.max_graph_size})"
                    )
    ...
    async def _handle_prompts_list(self, params: dict) -> dict:
        ...
        return {"jsonrpc": "2.0", "id": params.get("id"), "result": {"prompts": []}}
    ...
    async def run(self):
        ...
        self.audit.log_security_event(
            ...
            "low",
            {
                ...
                "auth_enabled": self.config.security.api_key_enabled
                or self.config.security.oauth_enabled,
                "rate_limiting": self.config.rate_limit.enabled,
            },
        )
    ...
    def shutdown(self):
        ...
        self.running = False
        self.audit.log_security_event(
            "server_shutdown", "low", {"reason": "graceful_shutdown"}
        )
