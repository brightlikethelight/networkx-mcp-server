...
class ResourceMonitor:
    ...
    def _get_memory_usage(self) -> int:
        ...
        try:
            ...
            return (
                len(self.active_operations) * 10 * 1024 * 1024
...
class RateLimiter:
    ...
    def check_rate_limit(self, user_id: str, operation: str = None) -> RateLimitResult:
        ...
        if not self.config.rate_limit.enabled:
            return RateLimitResult(
                allowed=True, remaining=999999, reset_time=time.time()
            )
        ...
        with self.bucket_lock, self.window_lock:
            ...
            return {
                "requests_per_minute": {
                    ...
                    if minute_bucket
                    else self.config.rate_limit.requests_per_minute,
                    "reset_time": time.time() + 60,
                },
                "requests_per_hour": {
                    "limit": self.config.rate_limit.requests_per_hour,
                    "remaining": hour_window.get_remaining()
                    if hour_window
                    else self.config.rate_limit.requests_per_hour,
    ...
    def reset_user_limits(self, user_id: str):
        ...
        with self.bucket_lock, self.window_lock, self.operation_lock:
            ...
            keys_to_remove = [
                key for key in self.operation_counters.keys() if key[0] == user_id
            ]
            for key in keys_to_remove:
                self.operation_counters.pop(key, None)

        self.audit.log_security_event(
            "rate_limit_reset", "low", {"user_id": user_id, "reset_by": "admin"}
        )
