...
from ..enterprise.auth import Role, User
from .threat_detection import PromptInjectionDetector, ThreatAssessment, ThreatLevel
from .validation import ValidationResult, ValidationStatus, ZeroTrustValidator
...
class SecurityBroker:
    ...
    async def authorize_operation(
        ...
        try:
            ...
            threat_assessment = self.threat_detector.detect_injection(
                prompt, {"tool_name": operation, "user_id": user.id, **context}
            )
            ...
            if authorization_result.risk_level in [
                OperationRisk.HIGH,
            ...
            self.logger.info(
                f"Authorization result: {authorization_result.authorized} "
    ...
    def _enforce_security_policies(
        ...
        for role in user.roles:
            if role in blocked_ops and operation in blocked_ops[role]:
                policy_violations.append(
                    f"Operation {operation} blocked for role {role.value}"
                )
        ...
        return {"violations": policy_violations, "actions": list(set(policy_actions))}
    ...
    async def _make_authorization_decision(
        ...
        requires_approval = (
            "REQUIRE_HUMAN_APPROVAL" in policy_result["actions"]
            or risk_level in [OperationRisk.HIGH, OperationRisk.CRITICAL]
            or any(
                role in self.security_policies["require_approval_for_roles"]
                for role in user.roles
            )
        )
        ...
        if requires_approval:
            approval_request = ApprovalRequest(
                ...
                + timedelta(seconds=self.security_policies["human_approval_timeout"]),
            )
    ...
    async def deny_request(
        ...
        return await self.approval_manager.deny_request(
    ...
    def get_security_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.security_stats,
            ...
            "high_risk_rate": self.security_stats["high_risk_operations"]
            / max(total, 1),
...
class OperationRiskAssessor:
    ...
    def assess_risk(
        ...
        # 3. Threat-based risk
        threat_multiplier = self.risk_factors["threat_level_multipliers"][
            threat_assessment.threat_level
        ]
        risk_score += threat_multiplier
        if threat_multiplier > 0.0:
            risk_factors.append(
                f"Threat detected: {threat_assessment.threat_level.value}"
            )

        # 4. Validation-based risk
        validation_multiplier = self.risk_factors["validation_multipliers"][
    ...
    def _generate_mitigation_actions(
        self, risk_level: OperationRisk, risk_factors: List[str]
    ) -> List[str]:
        ...
        if risk_level == OperationRisk.CRITICAL:
            ...
            actions.extend(
                ["Require human approval", "Enhanced monitoring", "Full audit logging"]
            )
        elif risk_level == OperationRisk.MEDIUM:
            actions.extend(["Enhanced monitoring", "Additional validation"])
        else:
