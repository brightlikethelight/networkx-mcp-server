...
from .validator import SecurityValidator, SecurityError, ValidationError
...
class SecureFileHandler:
        ...
        if allowed_dirs is None:
            ...
            for dir_path in allowed_dirs:
                try:
                    ...
                    raise FileSecurityError(f"Invalid allowed directory '{dir_path}': {e}")
    ...
    def validate_path(self, filepath: Union[str, Path]) -> Path:
        ...
        try:
            ...
            raise FileSecurityError(f"Invalid path '{filepath}': {e}")
        
        # Check for null bytes (path injection)
        if '\x00' in str(filepath):
            raise FileSecurityError("Path contains null bytes")
        ...
        if requested.exists() and requested.is_symlink():
            ...
            try:
                ...
                raise FileSecurityError("Cannot resolve symlink target")
        ...
        if '..' in path_str or path_str.count('/') > 20:
            raise FileSecurityError("Suspicious path pattern detected")
        
        # Check for hidden files (unless explicitly allowed)
        if requested.name.startswith('.') and requested.name != '.':
            raise FileSecurityError("Access to hidden files not allowed")
    ...
    def create_secure_temp_file(self, suffix: str = "", prefix: str = "graph_") -> Path:
        ...
        if suffix:
            ...
            if suffix not in safe_extensions:
                raise FileSecurityError(f"Unsafe file extension: {suffix}")
    ...
    def safe_read_graph(self, filepath: Union[str, Path], format: str) -> nx.Graph:
        ...
        if not secure_path.is_file():
            raise FileSecurityError("Path is not a file")
        ...
        try:
            if safe_format == 'graphml':
                ...
                raise FileSecurityError(f"Unsupported format: {safe_format}")
        except Exception as e:
            # Sanitize error message
            safe_error = SecurityValidator.sanitize_error_message(e)
            raise FileSecurityError(f"Failed to read graph: {safe_error}")
    ...
    def safe_write_graph(self, graph: nx.Graph, filepath: Union[str, Path], 
        ...
        try:
            ...
            # Sanitize error message
            safe_error = SecurityValidator.sanitize_error_message(e)
            raise FileSecurityError(f"Failed to write graph: {safe_error}")
    ...
    def _read_json_graph(self, filepath: Path) -> nx.Graph:
        ...
        try:
            ...
            raise FileSecurityError(f"Invalid JSON: {e}")
        
        # Validate structure
        if not isinstance(data, dict):
            raise FileSecurityError("JSON must be object at root")
    ...
    def _read_yaml_graph(self, filepath: Path) -> nx.Graph:
        ...
        if not isinstance(data, dict):
            raise FileSecurityError("YAML must be mapping at root")
        
        # Convert to graph
        if 'nodes' in data and 'edges' in data:
            return self._json_node_link_to_graph(data)
        else:
            raise FileSecurityError("Invalid YAML graph format")
