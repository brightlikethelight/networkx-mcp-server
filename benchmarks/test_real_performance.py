...
class RealBenchmark:
    ...
    async def test_node_scaling(self):
        ...
        for count in node_counts:
            try:
                ...
                # Create graph
                result = await self.call_tool("create_graph", {"graph_id": f"test_{count}"}, count * 10)
                if "error" in result["response"]:
                    self.results["errors"].append(f"Failed to create graph for {count} nodes: {result['response']['error']}")
                    break
                ...
                for i in range(0, count, batch_size):
                    ...
                    result = await self.call_tool("add_nodes", {
                        "graph_id": f"test_{count}",
                        "nodes": nodes
                    }, count * 10 + i)
                    
                    if "error" in result["response"]:
                        self.results["errors"].append(f"Failed to add nodes {i}-{batch_end}: {result['response']['error']}")
                        break
                ...
                self.results["node_limits"][count] = {
                    ...
                    "memory_per_node_kb": (end_memory - start_memory) * 1024 / count if count > 0 else 0
                }
                
                print(f"    ✓ {count} nodes: {end_time - start_time:.2f}s, {end_memory - start_memory:.1f}MB")
                ...
                self.results["errors"].append(f"Node scaling test failed at {count}: {e}")
        ...
        for count in edge_counts:
            try:
                ...
                for i in range(0, count, batch_size):
                    ...
                    result = await self.call_tool("add_edges", {
                        "graph_id": "edge_test",
                        "edges": batch_edges
                    }, 1000 + i)
                    
                    if "error" in result["response"]:
                        self.results["errors"].append(f"Failed to add edges {i}-{i+len(batch_edges)}: {result['response']['error']}")
                        break
                ...
                self.results["edge_limits"][count] = {
                    ...
                    "total_time_s": end_time - start_time,
                    "memory_mb": end_memory - baseline_memory,
                    "memory_per_edge_bytes": (end_memory - start_memory) * 1024 * 1024 / count if count > 0 else 0
                }
                
                print(f"    ✓ {count} edges: {end_time - start_time:.2f}s, {end_memory - start_memory:.1f}MB")
                ...
                self.results["errors"].append(f"Edge scaling test failed at {count}: {e}")
    ...
    async def test_algorithm_performance(self):
        ...
        # Test shortest path
        try:
            result = await self.call_tool("shortest_path", {
                "graph_id": "algo_test",
                "source": "node_0",
                "target": "node_50"
            }, 4)
        ...
        try:
            result = await self.call_tool("centrality_measures", {
                "graph_id": "algo_test",
    ...
    async def test_operation_timing(self):
        ...
        operations = [
            ...
            ("add_edges", {"graph_id": "timing_test", "edges": [["n1", "n2"], ["n2", "n3"]]}),
    ...
    def generate_report(self) -> str:
        ...
        report.append("**Testing Method**: Real subprocess communication with memory profiling")
        report.append("")
        ...
        for count, data in self.results["node_limits"].items():
            if data["success"]:
                report.append(f"| {count:,} | {data['total_time_s']:.2f} | {data['memory_mb']:.1f} | {data['memory_per_node_kb']:.1f} | ✓ |")
            else:
                report.append(f"| {count:,} | - | - | - | ✗ {data.get('error', 'Failed')} |")
        ...
        report.append("")
        report.append("| Edges | Time (s) | Memory (MB) | Memory/Edge (bytes) | Status |")
        report.append("|-------|----------|-------------|---------------------|---------|")
        
        for count, data in self.results["edge_limits"].items():
            if data["success"]:
                report.append(f"| {count:,} | {data['total_time_s']:.2f} | {data['memory_mb']:.1f} | {data['memory_per_edge_bytes']:.0f} | ✓ |")
            else:
                report.append(f"| {count:,} | - | - | - | ✗ {data.get('error', 'Failed')} |")
        ...
        # Analyze results
        max_nodes = max([k for k, v in self.results["node_limits"].items() if v["success"]], default=0)
        max_edges = max([k for k, v in self.results["edge_limits"].items() if v["success"]], default=0)
        ...
        if max_nodes > 0:
            ...
            report.append(f"- **Memory per node**: ~{node_data['memory_per_node_kb']:.1f} KB")
        
        if max_edges > 0:
            edge_data = self.results["edge_limits"][max_edges]
            report.append(f"- **Memory per edge**: ~{edge_data['memory_per_edge_bytes']:.0f} bytes")
        
        if self.results["errors"]:
            report.append(f"- **Errors encountered**: {len(self.results['errors'])}")
        
        report.append("")
        report.append("*This report contains actual measured performance data, not estimates.*")
